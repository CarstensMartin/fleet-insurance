...
// Utility function - Gets INSURANCE car data, and creates the file if it doesn't exist
function getInsuranceFleet() {
    try {
...
function deleteInsuranceCar(InsuranceCarIndex){
    const cars = getInsuranceFleet()
    cars.splice(InsuranceCarIndex, 1)
...
router.delete('/', (req, resp) => {
    ...
    const cars = getCars()
    const insuranceCars = getInsuranceFleet()

    // Find the index of the Insurance car to be deleted
    let InsuranceCarIndex = -1;
    for (i = 0; i < insuranceCars.length; i++) {
        if (insuranceCars[i].id == carId) {
            InsuranceCarIndex = i;
        }
    }
    
    // If index was found (Bigger than -1), action the delete otherwise let user know the ID does not exist
    if (InsuranceCarIndex > -1) {
        // Delete the car based on Index
        deleteInsuranceCar(InsuranceCarIndex)
    } 
